...
RSpec.describe Reservation, type: :model do
  ...
  context 'Validation' do
    it 'is valid with valid attributes' do
      reservation = Reservation.new(city: 'Apple', event_date: '10/10/2023', user:, vehicle:)
      expect(reservation).to be_valid
    end

    it 'is not valid without a city' do
      reservation = Reservation.new(event_date: '10/10/2023', user:, vehicle:)
      expect(reservation).to_not be_valid
    end

    it 'is not valid without an event_date' do
      reservation = Reservation.new(city: 'Apple', user:, vehicle:)
      expect(reservation).to_not be_valid
    end

    it 'is not valid without a user' do
      reservation = Reservation.new(city: 'Apple', event_date: '10/10/2023', vehicle:)
      expect(reservation).to_not be_valid
    end

    it 'is not valid without a vehicle' do
      reservation = Reservation.new(city: 'Apple', event_date: '10/10/2023', user:)
      expect(reservation).to_not be_valid
    end
  end
end
