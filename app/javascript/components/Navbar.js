...
const Navbar = () => {
  ...
  return (
    <div className={`bg-white fixed h-screen flex flex-col justify-start z-50 w-60 ${show ? "display" : "hide" }`}>
      <HiMenuAlt4 onClick={() => {toggleShow()}} className={`fixed text-primaryGreen left-3 top-3  text-3xl cursor-pointer ${show ? "display" : "hide" }`} />
